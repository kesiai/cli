...
- ⚠️ 两可时（设备资产的台账/档案表，含仪表、传感器、阀门等）：选 `device`——设备表同样能当纯台账用，而 `common` 表后续无法接入实时采集
...
2. **用户指定驱动名称时**（如"测试驱动"、"仿真驱动"、"Modbus"）：
   ...
   - 如果未安装，用 `$K driver-catalog --search <关键词>` 匹配驱动目录，**经用户确认后进入 [workflow-driver-create.md](workflow-driver-create.md) 自动创建安装，完成后带着新实例回到本阶段继续表规划**
3. **平台没有安装任何驱动时**：
   - 同样先查 `$K driver-catalog`，有合适驱动 → 征求用户意见走驱动创建子工作流；用户不要 → 使用测试驱动
   - 测试驱动的固定配置见下文"测试驱动配置"
...
**排查纪律：** 为定位问题临时建的探针表，得出结论后必须 `table-delete` 删除——最终交付的表清单只能含任务要求的表。
