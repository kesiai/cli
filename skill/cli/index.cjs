...
-  already used by option '${matchingOption.flags}'`);
        ...
        switch (parseOptions.from) {
          ...
          case "electron":
            if (process2.defaultApp) {
              ...
              userArgs2 = argv.slice(1);
            }
            break;
          case "user":
            userArgs2 = argv.slice(0);
            break;
          case "eval":
            userArgs2 = argv.slice(1);
            break;
      ...
      parse(argv, parseOptions) {
        ...
        const userArgs2 = this._prepareUserArgs(argv, parseOptions);
        this._parseCommand([], userArgs2);
        return this;
      }
      /**
       * Parse `argv`, setting options and invoking commands when defined.
      ...
      async parseAsync(argv, parseOptions) {
        this._prepareForParse();
        const userArgs2 = this._prepareUserArgs(argv, parseOptions);
        await this._parseCommand([], userArgs2);
        return this;
      }
      _prepareForParse() {
  ...
  "node_modules/crypto-js/core.js"(exports2, module2) {
    (function(root, factory2) {
      if (typeof exports2 === "object") {
        module2.exports = exports2 = factory2();
...
var require_sha1 = __commonJS({
  "node_modules/crypto-js/sha1.js"(exports2, module2) {
    (function(root, factory2) {
      if (typeof exports2 === "object") {
        module2.exports = exports2 = factory2(require_core());
      } else if (typeof define === "function" && define.amd) {
...
function readConfig() {
  const pick = (cliKey, envKey) => cliCreds[cliKey] || process.env[envKey];
  ...
  const cfg = { baseUrl, projectId, token };
...
function formatArrayTable(items) {
  ...
  const displayItems = items.map((item) => {
    ...
    for (const [k, v] of Object.entries(item)) {
      if (v === null || v === void 0) {
        ...
        const obj = v;
        row[k] = typeof obj.name === "string" ? obj.name : typeof obj.title === "string" ? obj.title : typeof obj.id === "string" ? obj.id : JSON.stringify(v);
  ...
  const rows = displayItems.map(
    (item) => keys.map((k) => String(item[k] ?? "").slice(0, 40).padEnd(colWidths[k])).join("  ")
  );
...
// src/core/errors.ts
var KesiError = class extends Error {
  code;
  details;
...
var ConfigError = class extends KesiError {
  constructor(message) {
    super(message, "CONFIG_ERROR");
  }
};
var AuthError = class extends KesiError {
  constructor(message, details) {
    super(message, "AUTH_ERROR", details);
  }
};
var ApiError = class extends KesiError {
  statusCode;
...
var NetworkError = class extends KesiError {
  constructor(message) {
...
var KesiApiClient = class {
  ...
  async doLogin() {
    const res = await this.http.post("/core/auth/login", {
      ...
      password: (0, import_sha1.default)(this.config.password).toString()
    });
    if (res.data?.token) {
  ...
  async login(username, password) {
    const res = await this.http.post("/core/auth/login", {
      username,
      password: (0, import_sha1.default)(password).toString()
    });
    return res.data;
  }
  // ==================== 表管理 ====================
  ...
  async getTableRecords(tableName, params) {
    ...
    const total = params?.withCount && res.headers["count"] ? parseInt(res.headers["count"], 10) : res.data?.length || 0;
  ...
  async saveTableRecord(tableName, data, upsert = false) {
    const res = await this.http.post(`/core/t/${tableName}/d`, data, {
      params: upsert ? { upsert: "true" } : void 0
    });
    if (res.data.InsertedID) return res.data.InsertedID;
    return res.data.id;
  }
  ...
  async deleteTableRecord(tableName, id, attachment = false) {
    await this.http.delete(`/core/t/${tableName}/d/${id}`, {
      params: attachment ? { attachment: "true" } : void 0
    });
  }
   ...
   * @param endTime 结束时间
   */
  async getHistoryData(pairs, startTime, endTime) {
...
`);
  ...
  // ==================== 设备控制 ====================
  /** 发送设备指令 */
  async sendDeviceCommand(tableId, deviceId, command, params) {
  ...
  async executeReport(id, parameters) {
  ...
  // ==================== 驱动 ====================
  async getDriverInstances(params) {
    const query = {
      sort: { createTime: -1 },
      skip: 0,
      limit: 50,
...
function normalizeQueryOptions(options) {
  return {
    filter: options.filter ? JSON.parse(options.filter) : void 0,
    sort: options.sort ? JSON.parse(options.sort) : void 0,
    limit: options.limit !== void 0 ? Number(options.limit) : void 0,
    skip: options.skip !== void 0 ? Number(options.skip) : void 0,
...
async function tableGet(id, options) {
  await executeCommand(async () => {
    const client = getApiClient();
...
async function tableCreate(options) {
  await executeCommand(async () => {
    let data;
    if (options.json) {
...
async function recordGet(tableName, id, options) {
  await executeCommand(async () => {
    const client = getApiClient();
...
async function recordCreate(tableName, options) {
  await executeCommand(async () => {
    let data;
    if (options.file) {
...
async function recordDelete(tableName, id, options = {}) {
  await executeCommand(async () => {
    const client = getApiClient();
...
async function recordsBatchDelete(tableName, ids) {
  await executeCommand(async () => {
    const client = getApiClient();
...
// src/commands/tag.ts
async function tagsList(tableId, options) {
  await executeCommand(async () => {
    const client = getApiClient();
...
async function recordTagsList(tableName, recordId, options) {
  await executeCommand(async () => {
    const client = getApiClient();
...
function normalizePairs(pairs, defaultTableId) {
  return pairs.map((p) => ({
...
async function dataLatest(options) {
  await executeCommand(async () => {
    let pairs;
    if (options.file) {
      pairs = JSON.parse((0, import_node_fs3.readFileSync)(options.file, "utf-8"));
    } else if (options.json) {
...
async function dataHistory(options) {
  await executeCommand(async () => {
...
async function warningRulesGet(id, options) {
  await executeCommand(async () => {
    const client = getApiClient();
...
async function warningRulesDelete(id) {
  await executeCommand(async () => {
    const client = getApiClient();
...
async function warningGet(id, options) {
  await executeCommand(async () => {
    const client = getApiClient();
...
async function warningsLatest(options) {
  await executeCommand(async () => {
    const client = getApiClient();
...
async function statsOnline(tableIds, options) {
  await executeCommand(async () => {
    const client = getApiClient();
...
async function fileUpload(filePath, options) {
  await executeCommand(async () => {
    const client = getApiClient();
...
async function controlSend(options) {
  await executeCommand(async () => {
    const client = getApiClient();
    const format = resolveOutputFormat(options.output);
    ...
    const cmd = commands.find((c) => c.name === options.command);
    if (!cmd) {
      throw new Error(`\u6307\u4EE4 '${options.command}' \u4E0D\u5B58\u5728\u3002\u53EF\u7528\u6307\u4EE4: ${commands.map((c) => c.name).join(", ") || "\u65E0"}`);
    }
...
function resolveParams(cmd, userParams) {
  ...
  if (!ioway || ioway === "\u9ED8\u8BA4\u5199\u5165") {
    ...
    for (const [key, field] of Object.entries(schema)) {
      const f = field;
      if (f.default !== void 0) {
        defaults2[key] = f.default;
      }
    }
    if (userParams) {
      return { ...defaults2, ...JSON.parse(userParams) };
    }
    return defaults2;
  }
  if (ioway === "\u8868\u5355\u5199\u5165") {
    if (!userParams) {
      const schema = writeIn.schema || {};
      const fields = Object.entries(schema).map(([key, f]) => `  ${key}: ${f.type || "string"}${f.title ? ` (${f.title})` : ""}${f.default !== void 0 ? ` \u9ED8\u8BA4=${f.default}` : ""}`).join("\n");
...
async function controlBatch(options) {
  await executeCommand(async () => {
    let commandsData;
    if (options.file) {
      commandsData = JSON.parse((0, import_node_fs5.readFileSync)(options.file, "utf-8"));
    } else if (options.json) {
    ...
    for (const item of commandsData) {
      ...
      const cmd = tableCommands.find((c) => c.name === item.command);
      if (!cmd) {
...
async function reportGet(id, options) {
  await executeCommand(async () => {
    const client = getApiClient();
...
async function reportExecute(id, options) {
  await executeCommand(async () => {
    let params;
    if (options.file) {
      params = JSON.parse((0, import_node_fs6.readFileSync)(options.file, "utf-8"));
    } else if (options.json) {
...
async function reportCreate(options) {
  await executeCommand(async () => {
    let data;
    if (options.file) {
      data = JSON.parse((0, import_node_fs6.readFileSync)(options.file, "utf-8"));
    } else {
...
async function driverGet(id, options) {
  await executeCommand(async () => {
    const client = getApiClient();
...
// src/commands/driver-schema.ts
async function driverSchema(driverType, options) {
  await executeCommand(async () => {
    const client = getApiClient();
...
async function queryGet(resource, id, options) {
  await executeCommand(async () => {
    const client = getApiClient();
      ...
      if (detail.template === "device" || detail.tableMajorType === "device") {
        ...
        try {
          ...
          if (rawCommands.length > 0) {
            entry.commands = rawCommands.map((cmd) => ({
              id: cmd.id,
              name: cmd.name,
...
function extractFields(schema) {
  ...
  return Object.entries(schema.properties).map(([key, field]) => ({
    ...
    enum: field.enum || void 0,
    enumNames: field.enumNames || void 0
  }));
}

// src/commands/ai/describe.ts
async function describe(tableId, options) {
  await executeCommand(async () => {
    const client = getApiClient();
    ...
    const fields = Object.entries(properties).map(([key, field]) => {
      const f = {
        key,
        title: field.title || key,
        type: field.type,
...
function buildQueryExamples(tableId, fields, _tags) {
  const examples = [];
  ...
  if (multiConditions.length >= 2) {
    const filter2 = "{" + multiConditions.join(",") + "}";
    examples.push({
      description: "\u591A\u6761\u4EF6\u7EC4\u5408\u8FC7\u6EE4",
      cli: `kesi records ${tableId} -f '${filter2}' -l 50`,
      api: `GET /core/t/${tableId}/d?query={"filter":${filter2},"limit":50,"withCount":true}`,
...
// src/commands/ai/sample.ts
async function sample(tableId, options) {
  await executeCommand(async () => {
    const client = getApiClient();
...
function validateTableSchema(tableSchema, index) {
  ...
  if (tableSchema.template === "device") {
    if (!tableSchema.function || !Array.isArray(tableSchema.function)) {
      warnings2.push(
        `\u26A0\uFE0F  [${title}] template="device" \u4F46\u7F3A\u5C11 function \u5B57\u6BB5\u3002\u8BBE\u5907\u8868\u5FC5\u987B\u8BBE\u7F6E "function": ["device", "warning", "computed"]\uFF0C\u5426\u5219\u540E\u7AEF\u4F1A\u8BC6\u522B\u4E3A\u666E\u901A\u8868\u3002`
      );
    } else if (!tableSchema.function.includes("device")) {
      warnings2.push(
        `\u26A0\uFE0F  [${title}] template="device" \u4F46 function \u6570\u7EC4\u4E2D\u4E0D\u5305\u542B "device"\u3002\u5F53\u524D function: ${JSON.stringify(tableSchema.function)}\uFF0C\u5E94\u5305\u542B "device"\u3002`
      );
    }
    if (!tableSchema.device) {
      warnings2.push(
        `\u26A0\uFE0F  [${title}] template="device" \u4F46\u7F3A\u5C11 device \u914D\u7F6E\u5757\u3002\u8BBE\u5907\u8868\u9700\u8981 device: { tags, settings, driver... } \u6765\u5B9A\u4E49\u6570\u636E\u70B9\u548C\u9A71\u52A8\u3002`
      );
    } else {
  ...
  if (expectedFn && tableSchema.function) {
    const missing = expectedFn.filter((f) => !tableSchema.function.includes(f));
    if (missing.length > 0) {
      warnings2.push(
        `\u26A0\uFE0F  [${title}] template="${tableSchema.template}" \u4F46 function \u7F3A\u5C11 ${JSON.stringify(missing)}\u3002\u5EFA\u8BAE\u8BBE\u7F6E "function": ${JSON.stringify(expectedFn)}\u3002`
      );
    }
  }
  return warnings2;
...
async function seed(options) {
  await executeCommand(async () => {
    ...
    for (let i = 0; i < data.tables.length; i++) {
      const warnings2 = validateTableSchema(data.tables[i].schema, i);
      if (warnings2.length > 0) {
        hasWarnings = true;
        warnings2.forEach((w) => console.error(w));
      }
    }
    if (hasWarnings) {
  ...
  - ${guard.errors.join("\n  - ")}`);
      ...
      if (table.records && table.records.length > 0) {
        ...
        console.error(`  \u2713 \u5DF2\u63D2\u5165 ${recordResults.records.filter((r) => r.status === "ok").length}/${table.records.length} \u6761\u8BB0\u5F55`);
      }
...
program2.name("kesi").description("KESI IoT \u5E73\u53F0\u547D\u4EE4\u884C\u5DE5\u5177").version("2.0.0");
function addOutput(cmd) {
  return cmd.option("-o, --output <format>", "\u8F93\u51FA\u683C\u5F0F: json, table, plain", "json");
}
program2.command("config").description("\u67E5\u770B\u89E3\u6790\u540E\u7684\u914D\u7F6E").action(showConfig);
addOutput(program2.command("tables").alias("tbl").description("\u67E5\u8BE2\u8868\u5217\u8868").option("-f, --filter <json>", "\u8FC7\u6EE4\u6761\u4EF6").option("-s, --sort <json>", "\u6392\u5E8F").option("-l, --limit <number>", "\u6570\u91CF\u9650\u5236", "50").option("--skip <number>", "\u8DF3\u8FC7\u6570\u91CF")).action(tablesList);
...
addOutput(program2.command("record-tags <table> <recordId>").description("\u67E5\u8BE2\u8BB0\u5F55\u5C5E\u6027\u70B9")).action(recordTagsList);
addOutput(program2.command("data-latest").description("\u67E5\u8BE2\u6700\u65B0\u6570\u636E").option("--table <id>", "\u8868ID").option("--device <id>", "\u8BBE\u5907/\u8BB0\u5F55ID").option("--tag <id>", "\u5C5E\u6027\u70B9ID").option("--file <path>", "\u4ECE\u6587\u4EF6\u8BFB\u53D6").option("--json <json>", "JSON \u6570\u636E\uFF08[{tableId,id,tagId}]\uFF09")).action(dataLatest);
addOutput(program2.command("data-history").description("\u67E5\u8BE2\u5386\u53F2\u6570\u636E").option("--table <id>", "\u8868ID").option("--device <id>", "\u8BBE\u5907/\u8BB0\u5F55ID").option("--tag <id>", "\u5C5E\u6027\u70B9ID").option("--file <path>", "\u4ECE\u6587\u4EF6\u8BFB\u53D6").option("--json <json>", "JSON \u6570\u636E\uFF08[{tableId,dataId,tagId}]\uFF09").requiredOption("--start <timestamp>", "\u5F00\u59CB\u65F6\u95F4\u6233(ms)").requiredOption("--end <timestamp>", "\u7ED3\u675F\u65F6\u95F4\u6233(ms)")).action(dataHistory);
addOutput(program2.command("stats-online <tableIds...>").description("\u8BBE\u5907\u5728\u7EBF\u7EDF\u8BA1")).action(statsOnline);
var rules = program2.command("rules").description("\u62A5\u8B66\u89C4\u5219\u7BA1\u7406");
addOutput(rules.command("list").description("\u67E5\u8BE2\u89C4\u5219\u5217\u8868").option("-f, --filter <json>", "\u8FC7\u6EE4\u6761\u4EF6").option("-l, --limit <number>", "\u6570\u91CF\u9650\u5236").option("--with-count", "\u8FD4\u56DE\u603B\u6570")).action(warningRulesList);
addOutput(rules.command("get <id>").description("\u83B7\u53D6\u89C4\u5219\u8BE6\u60C5")).action(warningRulesGet);
rules.command("create").description("\u521B\u5EFA\u89C4\u5219").requiredOption("-n, --name <name>", "\u89C4\u5219\u540D\u79F0").requiredOption("-l, --level <number>", "\u62A5\u8B66\u7EA7\u522B 1-4").option("-e, --enable <boolean>", "\u662F\u5426\u542F\u7528", "true").option("-d, --description <text>", "\u63CF\u8FF0").action(warningRulesCreate);
...
addOutput(program2.command("report <id>").description("\u62A5\u8868\u8BE6\u60C5")).action(reportGet);
addOutput(program2.command("report-execute <id>").description("\u6267\u884C\u62A5\u8868").option("--file <path>", "\u53C2\u6570\u6587\u4EF6").option("--json <json>", "\u53C2\u6570")).action(reportExecute);
...
addOutput(program2.command("scan").description("\u5BFC\u51FA\u5168\u5E93 schema manifest\uFF08AI \u4E13\u7528\uFF09").option("--with-sample", "\u9644\u5E26\u6837\u672C\u6570\u636E").option("-l, --limit <number>", "\u6BCF\u5F20\u8868\u6837\u672C\u6570\u91CF", "3").option("--skip-count", "\u8DF3\u8FC7 count \u67E5\u8BE2\uFF08\u5927\u6570\u636E\u91CF\u65F6\u52A0\u901F\uFF09").option("--output-file <path>", "\u8F93\u51FA\u5230\u6587\u4EF6")).action(scan);
addOutput(program2.command("describe <tableId>").description("\u8F93\u51FA\u5B57\u6BB5\u7EA7 schema\uFF08AI \u4E13\u7528\uFF09").option("--with-tags", "\u5305\u542B\u5C5E\u6027\u70B9")).action(describe);
addOutput(program2.command("sample <tableId>").description("\u5FEB\u901F\u9884\u89C8\u6570\u636E\uFF08AI \u4E13\u7528\uFF09").option("-l, --limit <number>", "\u6837\u672C\u6570\u91CF", "5")).action(sample);
program2.command("seed").description("\u6279\u91CF\u5EFA\u8868 + \u704C\u79CD\u5B50\u6570\u636E\uFF08AI \u4E13\u7528\uFF09").option("--file <path>", "seed JSON \u6587\u4EF6").option("--json <json>", "seed JSON \u6570\u636E").action(seed);
function extractCredentials(argv) {
  const rest = [];
  const c = {};
  let noMoreOpts = false;
  const isCred = (k) => k === "base-url" || k === "project-id" || k === "token" || k === "username";
...
var userArgs = extractCredentials(process.argv.slice(2));
program2.parse(userArgs, { from: "user" });
if (!userArgs.length) {
  program2.outputHelp();
}
